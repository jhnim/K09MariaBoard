...
<%
/*  
검색후 파라미터 처리를 위한 추가부분 : 리스트에서 검색 후 상세보기, 그리고 다시 리스트보기를 눌렀을때 검색이 유지되도록 
처리하기 위한 코드
*/
String queryStr = "";
...
<div class="row mt-3 mr-1">
			...
			<div class="row mb-3">
				...
				<%
				/*  
				로그인이 완료된 상태에서만 수정/삭제 버튼을 보이게하고, 또한 작성자에게만 노출되도록 한다.
				작성자가 아니라면 버튼은 숨김처리된다.
				*/
				if(session.getAttribute("USER_ID")!=null && session.getAttribute("USER_ID").toString().equals(dto.getId())){
				%>
				<!-- 게시물 수정하기는 특정게시물에 대해 수행되는 작업이므로 반드시 수정게시물의 일련번호(PK)가 파라미터로 전달되어야 한다.
				...
				<!--  
					게시물 삭제의 경우 로그인된 상태이므로 해당 게시물의 일려번호만 서버로 전송하면 된다.
